...
| ------------------ | ------------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| `dim_team`          | `team_name` (canonical spelling), `group_id` (FK), `confederation_id` (FK), `fifa_ranking` | `fifa_ranking` permanently `NULL` as of Build 6 — superseded by `team_tournament_ranking` (below), not populated here, since a returning team's ranking genuinely differs per tournament (`docs/decision_log.md`). 62 rows as of Build 5: the 2026 48 plus 14 teams that only appear in the historical comparison tournaments (`group_id` NULL for those 14 — no 2026 group applies) |
...
| `team_tournament_ranking` | `team_id` (FK), `tournament_id` (FK), `fifa_ranking`, `ranking_as_of_date` | Build 6. Grain: team × tournament, not a `dim_team` column — see `dim_team` row above. 104 rows (one per team per tournament), 9 with `fifa_ranking IS NULL` (teams not yet determined at that tournament's ranking-snapshot date — `docs/decision_log.md`) |
